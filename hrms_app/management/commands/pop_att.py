...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--user-id", type=int, help="Populate attendance for a specific user ID"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        users = (
            CustomUser.objects.filter(id=options["user_id"]).select_related(
                "personal_detail"
            )
            if options["user_id"]
            else CustomUser.objects.all().select_related("personal_detail")
        )

        # Parse date arguments and set date range
        from_date = (
            make_aware(datetime.strptime(options["from_date"], "%Y-%m-%d"))
            if options.get("from_date")
            else None
        )
        to_date = (
            make_aware(datetime.strptime(options["to_date"], "%Y-%m-%d"))
        ...
        for user in users:
            ...
            for date, log_times in result[emp_code].items():
                ...
                user_expected_logout_date_time = login_date_time + timedelta(
                ...
                rfrom_date_aware = (
                    kolkata_tz.localize(rfrom_date) if rfrom_date else None
                )
    ...
    def determine_attendance_status(
        ...
        if total_hours != 0:
            if (
                login_date_time.time() <= user_shift.grace_start_time
                and total_hours >= full_day_hours
                and logout_date_time.time() >= user_shift.end_time
            ):
                ...
                str_from_date = (
                    f"{login_date_time.date()} {user_shift.grace_start_time}"
                )
            ...
            elif self.is_early_going(
                login_date_time, logout_date_time, user_shift, user_expected_logout_time
            ):
                att_status = settings.HALF_DAY
                att_status_short_code = "H"
                ...
                if user_expected_logout_date_time.time() < user_shift.end_time:
                    ...
                    user_expected_logout_date_time = datetime.strptime(
                        str_to_date, "%Y-%m-%d %H:%M:%S"
                    )
    ...
    def is_late_coming(self, login_date_time, logout_date_time, user_shift):
        return login_date_time.time() >= user_shift.grace_start_time and (
            logout_date_time.time() < user_shift.grace_end_time
            or logout_date_time.time() > user_shift.end_time
        )

    def is_early_going(
        self, login_date_time, logout_date_time, user_shift, user_expected_logout_time
    ):
        return (
            login_date_time.time() <= user_shift.grace_start_time
